...
if day.weekday() >= 4:
    ...
    week_no = day.isocalendar()[1]
    week_no = week_dict[week_no]
    excel_file_path = r"C:\Users\jmu81\Lock_It_In\Are-You-Sharper-Than-A-Sportsbook\LIN_DB\SCORESWeek"+ week_no +".xlsx"
    output.to_excel(excel_file_path, index=False)
else:
    week_dict = dict({46:"10", 47:"11", 48:"12", 49:"13", 50:"14", 51:"15", 52:"16", 1:"17",2:"18"})
    week_no = day.isocalendar()[1]
    week_no = week_dict[week_no]
    scores = r"C:\Users\jmu81\Lock_It_In\Are-You-Sharper-Than-A-Sportsbook\LIN_DB\SCORESWeek"+ week_no +".xlsx"
    scores = pd.read_excel(scores)
    scores = pd.concat([scores, output], ignore_index=True)
    scores = scores.drop_duplicates(subset=['name'])
    scores.to_excel(r"C:\Users\jmu81\Lock_It_In\Are-You-Sharper-Than-A-Sportsbook\LIN_DB\SCORESWeek"+ week_no +".xlsx", index = False)
